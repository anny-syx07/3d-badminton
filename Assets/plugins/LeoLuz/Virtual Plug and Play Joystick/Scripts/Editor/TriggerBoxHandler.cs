...
public class TriggerHandler  {
    ...
    public static void Box(ref Bounds bounds, Vector3 RelativeToPosition, string content = "", bool flipX=false, bool flipY=false)
    {
        ...
        if (!Application.isPlaying)
        {
            var fmh_44_76_638997543101847970 = Quaternion.identity; Max = Handles.FreeMoveHandle(RelativeToPosition + originalMax, size, snap, Handles.RectangleHandleCap);
            var fmh_45_76_638997543101863310 = Quaternion.identity; Min = Handles.FreeMoveHandle(RelativeToPosition + originalMin, size, snap, Handles.RectangleHandleCap);
        }
        else
        {
        ...
        if (!Application.isPlaying)
            { var fmh_56_53_638997543101864670 = Quaternion.identity; MaxMin = Handles.FreeMoveHandle(MaxMin, size, snap, Handles.RectangleHandleCap); }

        Max.x = MaxMin.x;
        Min.y = MaxMin.y;
        Vector3 MinMax = new Vector3(Min.x, Max.y, Max.z); if (!Application.isPlaying)
            if (!Application.isPlaying)
                { var fmh_62_57_638997543101865830 = Quaternion.identity; MinMax = Handles.FreeMoveHandle(MinMax, size, snap, Handles.RectangleHandleCap); }
        Min.x = MinMax.x;
        Max.y = MinMax.y;
        ...
        EditorGUI.BeginChangeCheck();
        var fmh_100_84_638997543101867480 = Quaternion.identity; bounds.center = Handles.FreeMoveHandle(RelativeToPosition + bounds.center, size, snap, Handles.CircleHandleCap) - RelativeToPosition;
    ...
    public static void Circle(ref Vector3 point, ref float radius,  Vector3 RelativeToPosition, string content = "")
    {
        ...
        var fmh_112_75_638997543101869270 = Quaternion.identity; var newPoint = Handles.FreeMoveHandle(RelativeToPosition + point, size, snap, Handles.RectangleHandleCap);
